...
if (isset($_GET['toggle'])) {
    ...
    db()->update('tb_admin_sekolah', ['is_active' => !$admin['is_active']], 'id_admin_sekolah = :where_id', ['where_id' => (int) $_GET['toggle']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['id_admin']) && $_POST['id_admin']) {
        if (empty($_POST['password']))
            unset($data['password']);
        db()->update('tb_admin_sekolah', $data, 'id_admin_sekolah = :where_id', ['where_id' => (int) $_POST['id_admin']]);
                ...
                <form method="POST">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="nama_lengkap" class="form-control" required
                            value="<?= htmlspecialchars($editData['nama_lengkap'] ?? '') ?>">
                    </div>
                    ...
                    <div class="mb-3">
                        <label class="form-label">Password
                            <?= $editData ? '(kosongkan jika tidak diubah)' : '*' ?></label>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="jabatan" class="form-control" placeholder="cth: Operator PPDB"
                            value="<?= htmlspecialchars($editData['jabatan'] ?? '') ?>">
                    </div>

                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary"><i
                                class="bi bi-save me-2"></i><?= $editData ? 'Update' : 'Simpan' ?></button>
                        <?php if ($editData): ?>
                        ...
                        <tbody>
                            <?php foreach ($adminList as $admin): ?>
                                <tr>
                                    ...
                                    <td class="small"><?= htmlspecialchars(truncate($admin['nama_sekolah'] ?? '-', 25)) ?>
                                    </td>
                                    <td>
                                    ...
                                    <td>
                                        <a href="?edit=<?= $admin['id_admin_sekolah'] ?>"
                                            class="btn btn-sm btn-outline-primary"><i class="bi bi-pencil"></i></a>
                                        <a href="?toggle=<?= $admin['id_admin_sekolah'] ?>"
                                            class="btn btn-sm btn-outline-warning"><i
                                                class="bi bi-toggle-<?= $admin['is_active'] ? 'on' : 'off' ?>"></i></a>
                                        <a href="?delete=<?= $admin['id_admin_sekolah'] ?>"
                                            class="btn btn-sm btn-outline-danger"
                                            onclick="return confirm('Hapus admin ini?')"><i class="bi bi-trash"></i></a>
                                    </td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
